...
trigger_parse_arguments $(tg_get_command_arguments "$MESSAGE")
...
if [ "$TOKEN_JENKINS" = "" ]; then
  tg_send_message --chat_id "$(tg_get_chat_id "$MESSAGE")" --text "fatal: error: token Jenkins not available in variable" --reply_to_message_id "$(tg_get_message_id "$@")" --parse_mode "Markdown"
  exit
elif [ "$DEVICE" = "" ]; then
  trigger_help "$MESSAGE" "Missing, arguments -d (device) is required"
else
# Post api
  ...
  tg_send_message --chat_id "$(tg_get_chat_id "$MESSAGE")" --text "Successful trigger jenkins for $DEVICE.
We wish you a successful build and land safely" --reply_to_message_id "$(tg_get_message_id "$MESSAGE")" --parse_mode "Markdown"
fi
